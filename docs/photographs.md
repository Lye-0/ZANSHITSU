...
`public/images/` には採用したWebPだけを置く。現在214点。元PNG・候補・生成アトラス・検証画面は配信フォルダに含めない。
...
`src/photoAssets.ts` が全選択可能な写真のカタログ。`tests/photography.test.ts` がファイルの存在、未参照ファイル、24方向・44対象の輪郭を検証する。`docs/photographs.json` は全写真の寸法・容量・SHA-256。
